...
from ..config import SeriesLibrary
...
class SeriesCoverPickerDialog(QDialog):
    """Analog zu CoverPickerDialog bei Filmen, nur eben fuer Serien (eigener
    Cover-Ordner unter SeriesLibrary, siehe config.py)."""

    def __init__(self, series_library: SeriesLibrary, db: Database, series: Series, parent=None):
        ...
        self.setWindowTitle(f"Cover aendern – {series.display_title}")
        ...
        info_col.addWidget(QLabel(f"<b>{series.display_title}</b>"))
    ...
    def _load_current_preview(self):
        if self._series.poster_path:
            pm = QPixmap(str(self._series_library.dir_thumbs / self._series.poster_path))
            if not pm.isNull():
    ...
    def _open_search(self):
        query = self._series.display_title
        if self._series.first_air_year:
            query += f" {self._series.first_air_year}"
        url = "https://www.google.com/search?tbm=isch&q=" + query.replace(" ", "+") + "+serie+poster"
        webbrowser.open(url)
    ...
    def _save_cover(self, data: bytes):
        filename = f"{slugify(self._series.folder_name)}_{self._series.id}_manual.jpg"
        cover_path = self._series_library.dir_covers / filename
        thumb_path = self._series_library.dir_thumbs / filename
        ...
        self._db.set_series_manual_cover(self._series.id, filename)
